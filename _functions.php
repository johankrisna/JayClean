...
function add_kary($karyawan){
	...
	$telepon			= htmlspecialchars($karyawan['telepon']);
	...
	$insert = "INSERT INTO master VALUES ('','$nama', '$email','$username','$password','$level', '$telepon')";
...
function update_kary($up_kary){
	...
	$telepon 	= htmlspecialchars($up_kary['telepon']);
...
function changePassword($id_user, $newPassword){
    ...
    $updatePasswordQuery = "UPDATE master SET password = '$hashedPassword' WHERE id_user = '$id_user'";
    mysqli_query($koneksi, $updatePasswordQuery);
...
function add_ck($data_ck) {
    ...
    $durasi_ck   = htmlspecialchars($data_ck['durasi_ck']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
    ...
    $query_ck = "INSERT INTO tb_cuci_komplit VALUES (
        '', '$nama_pkt_ck', '$waktu_krj_ck', '$qty_ck', '$tarif_ck', '$durasi_ck'
    )";
...
function edit_ck($edit_ck) {
    ...
    $durasi_ck   = htmlspecialchars($edit_ck['waktu_kerja_unit']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
...
function add_dc($data_dc) {
    ...
    $durasi_dc   = htmlspecialchars($data_dc['durasi_dc']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
    ...
    $query_dc = "INSERT INTO tb_dry_clean VALUES (
        '', '$nama_pkt_dc', '$waktu_krj_dc', '$qty_dc', '$tarif_dc', '$durasi_dc'
    )";
...
function edit_dc($edit_dc) {
    ...
    $durasi_dc    = htmlspecialchars($edit_dc['waktu_kerja_unit']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
...
// CRUD Paket Cuci dalam
function add_cs($data_cs) {
    global $koneksi;

    $nama_pkt_cs  = htmlspecialchars($data_cs['nama_paket_cs']);
    $waktu_krj_cs = htmlspecialchars($data_cs['waktu_kerja_cs']);
    $durasi_cs   = htmlspecialchars($data_cs['durasi_cs']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
    ...
    $query_cs = "INSERT INTO tb_cuci_satuan VALUES (
        '', '$nama_pkt_cs', '$waktu_krj_cs', '$qty_cs', '$tarif_cs', '$durasi_cs'
    )";
...
function edit_cs($edit_cs) {
    ...
    $id_cs       = $edit_cs['id_cs'];
    $nama_pkt_cs  = htmlspecialchars($edit_cs['nama_paket_cs']);
    $waktu_krj_cs = htmlspecialchars($edit_cs['waktu_kerja_cs']);
    $durasi_cs   = htmlspecialchars($edit_cs['waktu_kerja_unit']); // Tambahkan untuk mendapatkan nilai waktu_kerja_unit
...
function order_ck($order_ck){
	...
	$ket = htmlspecialchars($order_ck['status']);
	...
	if (mysqli_num_rows($pkt_ck) === 1) {
		...
		/* Set nilai default untuk keterangan */
...
// Mendapatkan detail order cuci komplit berdasarkan nomor order
function get_order_ck_by_number($or_ck_number) {
    global $koneksi;
    $query = "SELECT * FROM tb_order_ck WHERE or_ck_number = '$or_ck_number'";
    $result = mysqli_query($koneksi, $query);
    return mysqli_fetch_assoc($result);
}
// Edit order cuci komplit berdasarkan nomor order
function edit_order_ck($order_ck, $or_ck_number) {
    global $koneksi;
    ...
    $wkt_kerja_ck = $result_ck['waktu_kerja_ck'];  // Sesuaikan nama variabel
    $tarif_perkilo = $result_ck['tarif_ck'];
    $total_bayar = $berat_qty * $result_ck['tarif_ck'];

    // Query update data order
    $update_ck = "UPDATE tb_order_ck SET
        nama_pel_ck='$nama_pel',
        no_telp_ck='$no_telp',
        alamat_ck='$alamat',
        jenis_paket_ck='$jns_pkt',
        waktu_kerja_ck='$wkt_kerja_ck',  
        berat_qty_ck='$berat_qty',
        tarif_perkilo_ck='$tarif_perkilo',
        tgl_masuk_ck='$tgl_masuk',
        tgl_keluar_ck='$tgl_keluar',
        total_bayar_ck='$total_bayar',
        keterangan_ck='$ket'
        WHERE or_ck_number='$or_ck_number'";

    mysqli_query($koneksi, $update_ck);
    return mysqli_affected_rows($koneksi);
}
    return 0; // Gagal update jika paket tidak ditemukan
}
...
function order_dc($order_dc){
	...
	$ket_dc = htmlspecialchars($order_dc['status']);
	...
	if (mysqli_num_rows($pkt_dc) === 1) {
		...
		$orderNum_dc = 'CL-' . strtoupper($limitNum);
	}
...
// Mendapatkan detail order dry clean berdasarkan nomor order
function get_order_dc_by_number($or_dc_number) {
    global $koneksi;
    $query = "SELECT * FROM tb_order_dc WHERE or_dc_number = '$or_dc_number'";
    $result = mysqli_query($koneksi, $query);
    return mysqli_fetch_assoc($result);
}

// Edit order dry clean berdasarkan nomor order
function edit_order_dc($order_dc, $or_dc_number) {
    global $koneksi;

    $nama_pel_dc = htmlspecialchars($order_dc['nama_pel_dc']);
    $no_telp_dc = htmlspecialchars($order_dc['no_telp_dc']);
    $alamat_dc = htmlspecialchars($order_dc['alamat_dc']);
    $jenis_paket_dc = htmlspecialchars($order_dc['jenis_paket_dc']);
    $berat_qty_dc = htmlspecialchars($order_dc['berat_qty_dc']);
    $tgl_masuk_dc = htmlspecialchars($order_dc['tgl_masuk_dc']);
    $tgl_keluar_dc = htmlspecialchars($order_dc['tgl_keluar_dc']);
    $keterangan_dc = htmlspecialchars($order_dc['keterangan_dc']);

    // Ambil data dari tabel daftar paket cuci kering (dry clean)
    $pkt_dc = mysqli_query($koneksi, "SELECT * FROM tb_dry_clean WHERE nama_paket_dc = '$jenis_paket_dc'");

    if (mysqli_num_rows($pkt_dc) === 1) {
        $result_dc = mysqli_fetch_assoc($pkt_dc);

        $waktu_kerja_dc = $result_dc['waktu_kerja_dc'];
        $tarif_dc = $result_dc['tarif_dc'];
        $total_bayar_dc = $berat_qty_dc * $result_dc['tarif_dc'];

        // Query update data order
        $update_dc = "UPDATE tb_order_dc SET
            nama_pel_dc='$nama_pel_dc',
            no_telp_dc='$no_telp_dc',
            alamat_dc='$alamat_dc',
            jenis_paket_dc='$jenis_paket_dc',
            waktu_kerja_dc='$waktu_kerja_dc',
            berat_qty_dc='$berat_qty_dc',
            tarif_dc='$tarif_dc',
            tgl_masuk_dc='$tgl_masuk_dc',
            tgl_keluar_dc='$tgl_keluar_dc',
            total_bayar_dc='$total_bayar_dc',
            keterangan_dc='$keterangan_dc'
            WHERE or_dc_number='$or_dc_number'";

        mysqli_query($koneksi, $update_dc);
        return mysqli_affected_rows($koneksi);
    }
    return 0; // Gagal update jika paket tidak ditemukan
}
...
function order_cs($order_cs){
	...
	$ket_cs = htmlspecialchars($order_cs['status']);
	...
	if (mysqli_num_rows($pkt_cs) === 1) {
		...
		$orderNum_cs = 'CD-' . strtoupper($limitNo_cs);
	}
...
// Mendapatkan detail order cuci satuan berdasarkan nomor order
function get_order_cs_by_number($or_cs_number) {
    global $koneksi;
    $query = "SELECT * FROM tb_order_cs WHERE or_cs_number = '$or_cs_number'";
    $result = mysqli_query($koneksi, $query);
    return mysqli_fetch_assoc($result);
}

// Edit order cuci satuan berdasarkan nomor order
function edit_order_cs($order_cs, $or_cs_number) {
    global $koneksi;

    $nama_pel_cs = htmlspecialchars($order_cs['nama_pel_cs']);
    $no_telp_cs = htmlspecialchars($order_cs['no_telp_cs']);
    $alamat_cs = htmlspecialchars($order_cs['alamat_cs']);
    $jenis_paket_cs = htmlspecialchars($order_cs['jenis_paket_cs']);
    $jml_pcs = htmlspecialchars($order_cs['jml_pcs']);
    $tgl_msk_cs = htmlspecialchars($order_cs['tgl_masuk_cs']);
    $tgl_kel_cs = htmlspecialchars($order_cs['tgl_keluar_cs']);
    $keterangan_cs = htmlspecialchars($order_cs['keterangan_cs']);

    // Ambil data dari tabel daftar paket cuci satuan
    $pkt_cs = mysqli_query($koneksi, "SELECT * FROM tb_cuci_satuan WHERE nama_cs = '$jenis_paket_cs'");

    if (mysqli_num_rows($pkt_cs) === 1) {
        $result_cs = mysqli_fetch_assoc($pkt_cs);

        $waktu_kerja_cs = $result_cs['waktu_kerja_cs'];
        $tarif_cs = $result_cs['tarif_cs'];
        $total_bayar_cs = $result_cs['tarif_cs'] * $jml_pcs;

        // Query update data order
        $update_cs = "UPDATE tb_order_cs SET
            nama_pel_cs='$nama_pel_cs',
            no_telp_cs='$no_telp_cs',
            alamat_cs='$alamat_cs',
            jenis_paket_cs='$jenis_paket_cs',
            waktu_kerja_cs='$waktu_kerja_cs',
            jml_pcs='$jml_pcs',
            tarif_cs='$tarif_cs',
            tgl_masuk_cs='$tgl_msk_cs',
            tgl_keluar_cs='$tgl_kel_cs',
            totBayar_cs='$total_bayar_cs',
            keterangan_cs='$keterangan_cs'
            WHERE or_cs_number='$or_cs_number'";

        mysqli_query($koneksi, $update_cs);
        return mysqli_affected_rows($koneksi);
    }
    return 0; // Gagal update jika paket tidak ditemukan
}
